...
&& pdnsutil import-zone-key nic.${2} keys/csk.txt \
&& pdnsutil show-zone nic.${2} 2>&1 | grep -e '^DS' | sed -e "s| *;.*||g" -e "s|nic\.${2}\.|nic|g" -e "s|.*= *|pdnsutil add-record ${2} |g" -e "s| IN DS *| DS 86400 '|g" -e "s|$|'|g" > update.sh \
&& bash update.sh \
